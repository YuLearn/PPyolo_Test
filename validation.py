...
def get_sen(y_true,y_pred,threshold=0.5):   #召回率
    y_pred = torch.sigmoid(y_pred)
    y_pred = (y_pred > threshold).float()
    epsilon = 1e-6
    TP = (((y_pred == 1) + (y_true==1))==2).float().sum(dim=-2).sum(dim=-1) #TP：判断正确，判为positive
    FN = (((y_pred == 0) + (y_true==1))==2).float().sum(dim=-2).sum(dim=-1) #FN: 判断错误，判为negative
    return list((TP/ (TP + FN + epsilon)).data.cpu().numpy())               #TP+FN:所有的正例
...
def get_spe(y_true,y_pred,threshold=0.5):
    ...
    FP = (((y_pred==1) + (y_true==0))==2).float().sum(dim=-2).sum(dim=-1)   #TN:判断正确，判为negative
    return list((TN / (TN + FP + epsilon)).data.cpu().numpy())              #TN+FP:所有负例

def get_ppv(y_true,y_pred,threshold=0.5):   #精确率
    y_pred = torch.sigmoid(y_pred)
    y_pred = (y_pred > threshold).float()
    epsilon = 1e-6
    TP = (((y_pred == 1) + (y_true==1))==2).float().sum(dim=-2).sum(dim=-1)
    FP = (((y_pred==1) + (y_true==0))==2).float().sum(dim=-2).sum(dim=-1)
    return list((TP / (TP + FP + epsilon)).data.cpu().numpy())              #TP+FP 所有判断出来的positive
